...
class TorchvisionTransform:
    def __init__(self, is_train: bool = True):
        ...
        common_transforms = [
            transforms.Resize((299, 299)),  # 이미지를 224x224 크기로 리사이즈
...
class AlbumentationsTransformTest:
    def __init__(self, is_train: bool = True):
        ...
        common_transforms = [
            A.Resize(299, 299),  # 이미지를 224x224 크기로 리사이즈
...
class TorchvisionModel(nn.Module):
    ...
    def __init__(
        ...
        if model_name == 'inception_v3':
            # pretrained와 aux_logits 설정
            weights = Inception_V3_Weights.IMAGENET1K_V1 if pretrained else None
            if weights is not None:
                aux_logits = True
            self.model = models.inception_v3(weights=weights, aux_logits=aux_logits)
        else:
...
#             inputs = self.processor(text=self.mini_values, images=images, return_tensors="pt", padding=True).to(self.device)
...
mini_imagenet_cls_map = {'n01872401': 'a photo of echidna, spiny anteater, anteater',
