...
class CLIPDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Union[Tuple[torch.Tensor, int], torch.Tensor]:
        ...
        text = str("a photo of ") + mini_imagenet_cls_map[classes]
        self.processor(text=text, images=image, return_tensors="pt", padding=True)
        if self.use_print:
            print("a photo of "+str(text))
        if self.is_inference:
            return self.processor(images=image, return_tensors="pt", padding=True).to(self.device)
        else:            
            return self.processor(text=text, images=image, return_tensors="pt", padding=True).to(self.device)
